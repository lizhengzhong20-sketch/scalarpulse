...
> **Pre-release note:** ScalarPulse is not on PyPI yet. Until the first package release, install the current version directly from this repository.
...
One JSONL line represents one atomic `log()` call. The format is append-friendly, human-readable, and straightforward to load into pandas.
